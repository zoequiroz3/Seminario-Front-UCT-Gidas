...
export default function ActividadDocenciaDetalle() {
  ...
  if (!data) return <p className="text-slate-500">No se encontró la actividad.</p>;
    ...
    <section className="flex flex-col gap-6">
      ...
      <article className="rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm">
        ...
        <div className="space-y-2 text-sm md:text-base text-slate-500">
          ...
          <p>
            <span className="font-medium text-slate-700">Fecha de finalización:</span>{" "}
            {formatFecha(data.fecha_fin)}
          </p>

          <p>
            <span className="font-medium text-slate-700">Grado Académico:</span>{" "}
